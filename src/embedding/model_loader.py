...
class ModelLoader:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if self.model:
            ...
            logger.info(f"Embedding model loaded: {self.model_name} (Dim: {self.embedding_dimension}) on {self.device}.")
        else:
    ...
    def _load_model(self) -> Optional['SentenceTransformer']:
        ...
        if not SENTENCE_TRANSFORMERS_AVAILABLE:
            logger.error("SentenceTransformers library not available. Install with: pip install sentence-transformers")
            return None
        ...
        try:
            ...
            model = SentenceTransformer(
                ...
                device=self.device,
                trust_remote_code=self.config['embedding'].get('trust_remote_code', False)
            )
            logger.info(f"Successfully loaded model {self.model_name}")
            return model
        except Exception as e:
            logger.critical(f"Failed to load embedding model '{self.model_name}' on device '{self.device}'. Error: {e}")
            logger.critical("Ensure the model name is correct and necessary dependencies (like PyTorch/TensorFlow) are installed.")
