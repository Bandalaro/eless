...
class Embedder:
    ...
    def __init__(self, config: Dict[str, Any], state_manager: StateManager, archiver: Archiver, 
                 model_loader: ModelLoader = None, resource_monitor=None, batch_processor: BatchProcessor = None):
        self.config = config
        ...
        logger.info(f"Embedder initialized with batch size: {self.batch_size}, "
    ...
    def _get_vectors_for_file(self, file_hash: str) -> Union[np.ndarray, None]:
        ...
        if vectors is not None:
            ...
            self.state_manager.add_or_update_file(file_hash, status=FileStatus.EMBEDDED, file_path='N/A (cached)')
            return vectors
        return None

    def embed_file_chunks(self, file_hash: str, chunks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if vectors.shape[0] != len(texts):
                logger.error(f"Vector count mismatch for {file_hash[:8]}. Expected {len(texts)}, got {vectors.shape[0]}")
                return []
            ...
            logger.info(f"File {file_hash[:8]}: Generated and cached {vectors.shape[0]} embeddings")
            
            return chunks
            
        except Exception as e:
            logger.error(f"Failed to embed chunks for file {file_hash[:8]}: {e}", exc_info=True)
            self.state_manager.add_or_update_file(file_hash, 'N/A (embedding failed)', FileStatus.ERROR)
            return []
    ...
    def _embed_with_adaptive_batching(self, texts: List[str]) -> np.ndarray:
        ...
        for batch_vectors in self.batch_processor.process_in_batches(
            texts, 
            self.model_loader.embed_chunks
        ):
    ...
    def embed_chunks_streaming(self, chunks_generator: Generator[Dict[str, Any], None, None], file_hash: str) -> Generator[Dict[str, Any], None, None]:
        """
        ...
        """
        chunk_batch = []
        batch_size = self.batch_processor.get_optimal_batch_size(self.batch_size) if self.batch_processor else self.batch_size
        
        logger.info(f"Starting streaming embedding for file {file_hash[:8]} with batch size {batch_size}")
        ...
        try:
            for chunk in chunks_generator:
                ...
                if len(chunk_batch) >= batch_size:
                    ...
                    if self.batch_processor:
                        batch_size = self.batch_processor.get_optimal_batch_size(batch_size)
            ...
            logger.error(f"Error in streaming embedding for file {file_hash[:8]}: {e}", exc_info=True)
            self.state_manager.add_or_update_file(file_hash, 'N/A (streaming embedding failed)', FileStatus.ERROR)
            
    def _process_chunk_batch(self, chunk_batch: List[Dict[str, Any]], file_hash: str) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if vectors.shape[0] != len(texts):
                logger.error(f"Vector count mismatch for batch from {file_hash[:8]}. Expected {len(texts)}, got {vectors.shape[0]}")
                return []
            ...
            logger.error(f"Failed to embed chunk batch from file {file_hash[:8]}: {e}", exc_info=True)
            return []
