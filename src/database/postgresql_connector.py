...
class PostgreSQLConnector(DBConnectorBase):
    ...
    def connect(self):
        if not self.dsn: raise ConnectionError("PostgreSQL DSN not configured.")
        try:
            ...
            logger.info(f"PostgreSQL connection successful. Table '{self.table_name}' ready.")
    ...
    def upsert_batch(self, vectors: List[Dict[str, Any]]):
        if not self.conn: raise ConnectionError("PostgreSQL connection not initialized.")
        ...
        try:
            with self.conn.cursor() as cur:
                ...
                self.conn.commit()
                logger.debug(f"Successfully upserted {len(vectors)} vectors to PostgreSQL.")
