...
try:
    ...
    logger.warning("ChromaDB connector not available - install chromadb and langchain-community")
...
class DatabaseLoader:
    ...
    def __init__(self, config: Dict[str, Any], state_manager: StateManager, embedding_model):
        self.config = config
        ...
        if embedding_model and hasattr(embedding_model, 'get_sentence_embedding_dimension'):
            self.embedding_dimension = embedding_model.get_sentence_embedding_dimension()
        else:
            self.embedding_dimension = config['embedding']['dimensions']
        
        logger.info(f"DatabaseLoader initializing with dimension: {self.embedding_dimension}")
        ...
        logger.info(f"DatabaseLoader initialized. Active targets: {list(self.active_connectors.keys())}")
    ...
    def _initialize_connectors(self):
        ...
        for name in target_names:
            ...
            if not connection_config:
                logger.error(f"Configuration for database connection '{name}' not found in 'connections'. Skipping.")
                continue
            
            db_type = connection_config.get('type')

            if not db_type:
                logger.error(f"Connection '{name}' is missing the required 'type' field. Skipping.")
                continue
            ...
            if db_type in CONNECTOR_MAP:
                try:
                    ...
                    connector = connector_class(self.config, name, self.embedding_dimension)
                    ...
                    if connector.check_connection():
                        ...
                        logger.error(f"✗ Connector '{name}' failed connection check after setup")

                except Exception as e:
                    logger.error(f"✗ Failed to initialize/connect to DB '{name}' (Type: {db_type}): {e}", exc_info=True)
            else:
                logger.error(f"Unknown database type '{db_type}' configured for connection '{name}'. Available types: {list(CONNECTOR_MAP.keys())}")
                
        logger.info(f"Database connector initialization complete. Active: {len(self.active_connectors)}/{len(target_names)}")


    def load_data(self, embedded_chunk_generator: Generator[Dict[str, Any], None, None]):
        """
        ...
        if not self.active_connectors:
            logger.warning("No active database connections. Data will not be persisted.")
            return

        batch: List[Dict[str, Any]] = []
        # Use a set to track file hashes that were processed in this run
        file_hashes_in_run: Dict[str, bool] = {} # {file_hash: True if all DBs upserted}
    ...
    def _upsert_batch(self, batch: List[Dict[str, Any]], file_hashes_in_run: Dict[str, bool]):
        """Sends the batch to all active database connectors and tracks success."""
        logger.info(f"Upserting batch of size {len(batch)} to {len(self.active_connectors)} targets.")
    ...
    def _update_file_status(self, file_hashes_in_run: Dict[str, bool]):
        ...
        for file_hash, success in file_hashes_in_run.items():
            if success:
                ...
                self.state_manager.add_or_update_file(file_hash, 
                                                     status=FileStatus.LOADED, 
                                                     file_path='N/A (DB loaded)')
                ...
                logger.warning(f"File {file_hash[:8]} failed to load to one or more databases. Status reverted to EMBEDDED.")
