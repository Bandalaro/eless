...
class ParallelProcessor:
    ...
    def __init__(self, config: Dict[str, Any], resource_monitor: ResourceMonitor = None):
        self.config = config
        ...
        self.parallel_config = ParallelConfig(
            ...
            enable_parallel_embedding=parallel_config.get('enable_parallel_embedding', True),
            enable_parallel_database=parallel_config.get('enable_parallel_database', True),
        ...
        logger.info(f"ParallelProcessor initialized - Mode: {self.parallel_config.processing_mode}, "
    ...
    def adjust_worker_count(self) -> int:
        ...
        with self.worker_lock:
            ...
            if should_throttle:
                ...
                logger.info(f"Reducing workers from {old_workers} to {self.current_workers} - {reason}")
            elif (metrics.memory_percent < 60 and 
                  metrics.cpu_percent < 70 and 
                  self.current_workers < self.max_workers):
                # Increase workers when resources are available
                self.current_workers = min(self.max_workers, self.current_workers + 1)
                logger.info(f"Increasing workers from {old_workers} to {self.current_workers} - abundant resources")
        
        return self.current_workers
    
    def process_files_parallel(self, file_metadata_list: List[Dict], 
                             processor_func: Callable, 
                             progress_callback: Callable = None) -> Iterator[Dict]:
        """
        ...
        """
        if not self.parallel_config.enable_parallel_files or len(file_metadata_list) <= 1:
            logger.info("Processing files sequentially")
            for file_meta in file_metadata_list:
                yield processor_func(file_meta)
            return
        
        logger.info(f"Processing {len(file_metadata_list)} files with {self.current_workers} workers")
        ...
        with executor_class(max_workers=self.current_workers) as executor:
            ...
            future_to_file = {}
            for file_meta in file_metadata_list:
                future = executor.submit(self._safe_file_processor, processor_func, file_meta)
            ...
            for future in as_completed(future_to_file):
                ...
                try:
                    ...
                    logger.error(f"Failed to process file {file_meta.get('path', 'unknown')}: {e}")
    ...
    def _safe_file_processor(self, processor_func: Callable, file_meta: Dict) -> Dict:
        ...
        try:
            ...
            with self.stats_lock:
                ...
                new_avg = (current_avg * total_files + processing_time) / (total_files + 1)
    ...
    def process_chunks_parallel(self, chunks: List[Dict], 
        ...
        """
        if not self.parallel_config.enable_parallel_chunks or len(chunks) <= self.parallel_config.chunk_batch_size:
        ...
        logger.info(f"Processing {len(chunks)} chunks in parallel batches of {batch_size}")
        ...
        processed_chunks = []
        with executor_class(max_workers=min(self.current_workers, len(chunk_batches))) as executor:
            ...
            for future in as_completed(future_to_batch):
                try:
                    ...
                    with self.stats_lock:
                        self.processing_stats['chunks_processed'] += len(future_to_batch[future])
    ...
    def process_embeddings_parallel(self, text_batches: List[List[str]], 
                                   embedder_func: Callable) -> List:
        """
        ...
        logger.info(f"Generating embeddings for {len(text_batches)} batches in parallel")
        
        # For embeddings, prefer thread-based parallelism due to model sharing
        all_embeddings = []
        with ThreadPoolExecutor(max_workers=min(self.current_workers, len(text_batches))) as executor:
            future_to_batch = {
                executor.submit(embedder_func, batch): batch 
                for batch in text_batches
            }
            ...
            for future in as_completed(future_to_batch):
                try:
                    ...
                    with self.stats_lock:
                        self.processing_stats['embeddings_generated'] += len(batch_embeddings)
    ...
    def load_to_databases_parallel(self, db_connectors: Dict, 
                                  batch_data: List[Dict]) -> Dict[str, bool]:
        """
        ...
        results = {}
        with ThreadPoolExecutor(max_workers=min(self.current_workers, len(db_connectors))) as executor:
            future_to_db = {
