...
class StreamingDocumentProcessor:
    ...
    def __init__(self, config: Dict[str, Any]):
        self.config = config
        self.max_memory_usage_mb = config.get('resource_limits', {}).get('max_memory_mb', 512)
        self.chunk_buffer_size = config.get('streaming', {}).get('buffer_size', 8192)  # 8KB chunks
        self.max_file_size_mb = config.get('streaming', {}).get('max_file_size_mb', 100)  # 100MB limit
        
        logger.info(f"StreamingProcessor initialized - Max memory: {self.max_memory_usage_mb}MB, "
    ...
    def process_large_text_file(self, file_path: Path, file_hash: str, 
                               chunker_func) -> Iterator[Dict[str, Any]]:
        """
        ...
        try:
            for file_chunk in self.stream_text_chunks(file_path):
                ...
                if len(text_buffer) >= buffer_limit:
                    ...
                    break_point = self._find_good_break_point(text_buffer, buffer_limit // 2)
    ...
    def estimate_memory_usage(self, file_path: Path) -> float:
        ...
        try:
            ...
            estimated_chunks = (file_size_mb * 1024) // self.config['chunking']['chunk_size']
            ...
            logger.debug(f"Memory estimate for {file_path.name}: {total_estimate:.1f}MB "
    ...
    def should_use_streaming(self, file_path: Path) -> bool:
        ...
        if should_stream:
            logger.info(f"Using streaming for {file_path.name} "
                       f"(estimated: {estimated_memory:.1f}MB > available: {available_memory:.1f}MB)")
        else:
            logger.debug(f"Using regular processing for {file_path.name} "
                        f"(estimated: {estimated_memory:.1f}MB <= available: {available_memory:.1f}MB)")
...
class BatchProcessor:
    ...
    def __init__(self, config: Dict[str, Any], resource_monitor):
        ...
        logger.info(f"BatchProcessor initialized - Base: {self.base_batch_size}, "
    ...
    def get_optimal_batch_size(self, total_items: int) -> int:
        ...
        recommended_size, _ = self.resource_monitor.get_adaptive_batch_size(self.base_batch_size)
