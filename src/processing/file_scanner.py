...
class FileScanner:
    ...
    def _generate_file_hash(self, file_path: Path) -> str:
        ...
        try:
            ...
            logger.error(f"Error reading file {file_path} for hashing: {e}", exc_info=True)
            raise

    @log_performance('ELESS.FileScanner')
    def scan_input(self, input_path: Union[str, Path]) -> List[Dict[str, Union[str, Path]]]:
        """
        ...
        if input_path.is_file():
            ...
            if input_path.suffix.lower() in self.supported_extensions:
                ...
                logger.warning(f"File {input_path.name} skipped: Unsupported extension {input_path.suffix}")
            ...
            for root, _, files in os.walk(input_path):
                ...
                for file_name in files:
                    ...
                    if file_path.suffix.lower() in self.supported_extensions:
                        ...
                        logger.debug(f"Skipping file {file_path.name}: Unsupported extension {file_path.suffix}")
                        skipped_files += 1
                        
            logger.info(f"Directory scan complete. Total: {total_files}, Found: {len(files_found)}, Skipped: {skipped_files}, Errors: {error_files}")
        else:
