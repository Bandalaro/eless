...
def parse_csv(file_path: Path) -> str:
    ...
    try:
        ...
        logger.error(f"An unexpected error occurred while parsing {file_path.name} as CSV: {e}")
...
def parse_xlsx(file_path: Path) -> str:
    ...
    try:
        ...
        df = pl.read_excel(file_path, sheet_name=0, read_options={"infer_schema_length": 100})
        ...
        logger.error(f"An unexpected error occurred while parsing {file_path.name} as Excel: {e}")
