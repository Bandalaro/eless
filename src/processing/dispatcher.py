...
class Dispatcher:
    ...
    def __init__(self, config: Dict[str, Any], state_manager: StateManager, archiver: Archiver, resource_monitor=None):
        self.config = config
        ...
        logger.info(f"Dispatcher initialized with parsers for: {list(self.parser_map.keys())}")
    ...
    def process_document(self, file_data: Dict[str, Any]) -> Generator[Dict[str, Any], None, None]:
        """
        ...
        if raw_text_chunks is None:
            ...
            self.state_manager.add_or_update_file(file_hash, str(file_path), FileStatus.SCANNED)
            logger.info(f"Processing new file: {file_path.name}...")
            
            # Check if we should use streaming processing
            if self.streaming_processor.should_use_streaming(file_path) and extension in ['.txt', '.md']:
                ...
                if not raw_text:
                    self.state_manager.add_or_update_file(file_hash, str(file_path), FileStatus.ERROR, metadata={'error': 'Failed to extract text.'})
                    return
            ...
            if not raw_text_chunks:
                self.state_manager.add_or_update_file(file_hash, str(file_path), FileStatus.ERROR, metadata={'error': 'No chunks created.'})
                return
            
            # Save chunks and update status
            self.archiver.save_chunks(file_hash, raw_text_chunks)
            self.state_manager.add_or_update_file(file_hash, str(file_path), FileStatus.CHUNKED)
            logger.info(f"File {file_hash[:8]} chunked ({len(raw_text_chunks)} chunks) and saved to cache.")
    ...
    def parse_and_chunk(self, file_path: Path, file_meta: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if self.streaming_processor.should_use_streaming(file_path) and extension in ['.txt', '.md']:
                ...
                logger.info(f"Created {len(chunks)} chunks using streaming from {file_path.name}")
                return chunks
            else:
                # Traditional processing for smaller files or non-text files
                raw_text = self._get_raw_text(file_path, extension)
            ...
            logger.error(f"Error parsing and chunking {file_path.name}: {e}", exc_info=True)
            return []
