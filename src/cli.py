...
def get_default_config_path():
    ...
    dev_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'config', 'default_config.yaml')
...
@click.option('--cache-dir', type=click.Path(), help='Set custom cache directory for processed data')
@click.option('--data-dir', type=click.Path(), help='Set custom data directory (parent for cache, logs, and databases)')
@click.pass_context
...
@click.option('--databases', '-db', multiple=True, 
...
@click.option('--parallel-workers', type=int, help='Number of parallel workers (0 to disable parallel processing)')
@click.option('--parallel-mode', type=click.Choice(['thread', 'process', 'auto', 'disable']), 
              help='Parallel processing mode: thread, process, auto, or disable')
@click.option('--disable-parallel-files', is_flag=True, help='Disable parallel file processing')
@click.option('--disable-parallel-embedding', is_flag=True, help='Disable parallel embedding generation')
@click.option('--disable-parallel-db', is_flag=True, help='Disable parallel database operations')
@click.pass_context
def process(ctx, source, config, resume, databases, chunk_size, batch_size, 
            parallel_workers, parallel_mode, disable_parallel_files, 
            disable_parallel_embedding, disable_parallel_db):
    """
    ...
    try:
        ...
        config_path = Path(config) if config else (Path(DEFAULT_CONFIG_PATH) if DEFAULT_CONFIG_PATH else None)
        ...
        if any([parallel_workers is not None, parallel_mode, disable_parallel_files, 
                disable_parallel_embedding, disable_parallel_db]):
            ...
            if parallel_workers is not None:
                if parallel_workers == 0:
                    ...
                    cli_overrides['parallel_processing']['max_workers'] = parallel_workers
            ...
            if disable_parallel_embedding:
                cli_overrides['parallel_processing']['enable_parallel_embedding'] = False
        ...
        if ctx.obj.get('data_dir'):
            ...
            if not ctx.obj.get('log_dir'):
                cli_overrides['logging']['directory'] = str(Path(data_dir) / '.eless_logs')
            if not ctx.obj.get('cache_dir'):
                cli_overrides['cache']['directory'] = str(Path(data_dir) / '.eless_cache')
    ...
    try:
        ...
        embedder = Embedder(app_config, state_manager, archiver, model_loader, resource_monitor)
        ...
        if not db_connectors:
            click.secho("No database connections available. Check your configuration.", fg='red')
            logger.error("No active database connections")
            return
        
        logger.info(f"Active database connections: {list(db_connectors.keys())}")
        click.echo(f"Connected to {len(db_connectors)} database(s): {', '.join(db_connectors.keys())}")
        ...
        for i, file_meta in enumerate(file_metadata_list):
            file_id = file_meta.get('file_id') or file_meta.get('hash') # Support both formats
            file_path = file_meta['path']
            status = state_manager.get_status(file_id)
            
            click.echo(f"\n--- Processing File {i+1}/{total_files}: {os.path.basename(file_path)} (Status: {status}) ---")
            logger.info(f"Processing file {i+1}/{total_files}: {file_path} (Status: {status})")
            ...
            try:
                ...
                if status == 'PENDING' or (status == 'EMBEDDED' and not resume):
                    ...
                    logger.info(f"Estimated memory for {os.path.basename(file_path)}: {estimated_memory:.1f}MB")
                    ...
                    click.secho("Status is EMBEDDED. Resuming from Vector Loading step.", fg='yellow')
                    ...
                    click.secho("Status is CHUNKED. Resuming from Embedding step.", fg='yellow')
                    logger.info("Resuming from CHUNKED status")
                    # Load chunks from cache
                    chunks = archiver.load_chunks(file_id) or []

                if not chunks:
                    click.secho(f"Could not extract chunks from {file_path}. Skipping.", fg='red')
                ...
                if status != 'EMBEDDED':
                    ...
                    try:
                        ...
                        if not embedded_chunks:
                            click.secho(f"Failed to generate embeddings for {file_path}", fg='red')
                            error_count += 1
                            continue
                            
                        # Extract vectors for database loading
                        vectors = [chunk['vector'].tolist() if hasattr(chunk['vector'], 'tolist') else chunk['vector'] for chunk in embedded_chunks]
                    ...
                    if cached_vectors is not None:
                        vectors = cached_vectors.tolist() if hasattr(cached_vectors, 'tolist') else cached_vectors
                        logger.info(f"Loaded {len(vectors)} cached vectors for {file_path}")
                    else:
                        logger.warning(f"Status was EMBEDDED but no cached vectors found for {file_path}")
                        continue
                        ...
                        for idx, (chunk, vector) in enumerate(zip(chunks, vectors)):
                            batch_data.append({
                                ...
                                'metadata': {
                                    ...
                                    'file_path': file_path,
                                    'chunk_index': idx,
                                    'text': chunk['text'][:200],  # Truncated text for metadata
                                    **chunk.get('metadata', {})
                                }
                            })
                        
                        connector.upsert_batch(batch_data)
                        click.secho(f"    ✓ Loaded successfully into {name}", fg='green')
                        logger.info(f"Successfully loaded into {name}")
                    except Exception as db_e:
                        click.secho(f"    ✗ Failed to load into {name}: {db_e}", fg='red')
                ...
                if all_success:
                    ...
                    click.secho(f"File {os.path.basename(file_path)} successfully LOADED.", fg='green')
    ...
    try:
        ...
        if all:
            ...
            for hash_id in all_hashes:
                ...
                color = 'green' if file_status == 'LOADED' else 'yellow' if file_status in ['CHUNKED', 'EMBEDDED'] else 'red'
                click.secho(f"ID: {hash_id[:12]}... | Status: {file_status:>10} | {os.path.basename(file_path)}", fg=color)
            ...
            for status, count in sorted(status_counts.items()):
                color = 'green' if status == 'LOADED' else 'yellow' if status in ['CHUNKED', 'EMBEDDED'] else 'red'
...
@click.option('--config', type=click.Path(exists=True), default=None,
              help='Path to a custom configuration file.')
@click.option('--test-db', type=click.Choice(AVAILABLE_DATABASES, case_sensitive=False),
    ...
    try:
        ...
        try:
            ...
            if embedding_model:
                ...
                test_vectors = embedding_model.encode(["This is a test sentence."], convert_to_tensor=False)
                click.secho("   ✓ Embedding model loaded and tested successfully", fg='green')
                ...
                for name, connector in db_loader.active_connectors.items():
                    try:
                        if connector.check_connection():
                            click.secho(f"   ✓ {name} connection successful", fg='green')
    ...
    try:
        ...
        if log_files:
            ...
            for log_file in sorted(log_files):
                ...
                import datetime
                mod_date = datetime.datetime.fromtimestamp(modified_time).strftime('%Y-%m-%d %H:%M:%S')
...
def cache(ctx, config, stats, cleanup, evict, clear):
    ...
    try:
        ...
        if stats:
            ...
            click.echo(f"Current Size: {stats_data['size_mb']:.1f} MB / {stats_data['max_size_mb']} MB")
            ...
            if corrupted:
                click.secho(f"Cleaned {len(corrupted)} corrupted cache entries", fg='green')
            else:
            ...
            # Clear entire cache
            if click.confirm("WARNING: This will delete all cached data. Are you sure?"):
            ...
            stats_data = cache_mgr.get_cache_stats()
            click.echo(f"Cache: {stats_data['size_mb']:.1f}MB/{stats_data['max_size_mb']}MB ({stats_data['utilization_percent']:.1f}%)")
...
@config.command()
@click.option('--output', '-o', type=click.Path(), default='eless_config.yaml',
...
@config.command()
@click.argument('preset', type=click.Choice(['minimal', 'standard', 'high-end', 'docker']))
@click.option('--output', '-o', type=click.Path(), default='eless_config.yaml',
              help='Output configuration file path')
@click.pass_context
...
def validate(ctx, config_file):
    ...
    try:
        ...
        if missing_sections:
            click.secho(f"❌ Missing required sections: {', '.join(missing_sections)}", fg='red')
            return
        ...
        if not (1 <= batch_size <= 1000):
            click.secho(f"⚠️  Warning: batch_size {batch_size} may be too large/small", fg='yellow')
        
        cache_size = config_dict.get('cache', {}).get('max_size_mb', 1024)
        if cache_size < 100:
            click.secho(f"⚠️  Warning: cache size {cache_size}MB may be too small", fg='yellow')
...
@click.option('--output', '-o', type=click.Path(), default='auto_config.yaml',
              help='Output configuration file path')
@click.pass_context  
def auto_detect(ctx, output):
    ...
    try:
        ...
        click.echo(f"🔍 Detected system: {memory_gb:.1f}GB RAM, {cpu_cores} CPU cores, {disk_gb:.1f}GB free disk")
        ...
        if memory_gb > 16:
            config_dict['embedding']['batch_size'] = min(128, config_dict['embedding']['batch_size'] * 2)
            config_dict['cache']['max_size_mb'] = min(8192, config_dict['cache']['max_size_mb'] * 2)
        ...
        click.echo(f"   Memory warning: {config_dict['resource_limits']['memory_warning_percent']}%")
...
def monitor(ctx, config, interval, duration):
    ...
    try:
        ...
        while True:
            ...
            memory_color = 'red' if summary['memory_percent'] > 90 else 'yellow' if summary['memory_percent'] > 80 else 'green'
            click.secho(f"   Memory: {summary['memory_percent']:.1f}% ({summary['memory_pressure']}) - {summary['memory_available_mb']:.0f}MB free", fg=memory_color)
            
            # CPU status
            cpu_color = 'red' if summary['cpu_percent'] > 90 else 'yellow' if summary['cpu_percent'] > 80 else 'green'
            click.secho(f"   CPU: {summary['cpu_percent']:.1f}% - Trend: {summary['cpu_trend']}", fg=cpu_color)
            
            # Disk usage
            disk_color = 'red' if summary['disk_usage_percent'] > 90 else 'yellow' if summary['disk_usage_percent'] > 80 else 'green'
            click.secho(f"   Disk: {summary['disk_usage_percent']:.1f}% used", fg=disk_color)
            ...
            click.echo(f"   Recommended batch size: {summary['recommended_batch_size']}")
            
            # Cache status
            cache_stats = cache_manager.get_cache_stats()
            click.echo("\n💾 Cache Status:")
            cache_color = 'red' if cache_stats['utilization_percent'] > 90 else 'yellow' if cache_stats['utilization_percent'] > 75 else 'green'
            click.secho(f"   Size: {cache_stats['size_mb']:.1f}MB / {cache_stats['max_size_mb']}MB ({cache_stats['utilization_percent']:.1f}%)", fg=cache_color)
            click.echo(f"   Files: {cache_stats['file_count']} / {cache_stats['max_files']}")
            
            # Performance recommendations
            click.echo("\n💡 Recommendations:")
            if summary['memory_percent'] > 85:
                click.secho("   • Consider reducing batch size or clearing cache", fg='yellow')
            if cache_stats['utilization_percent'] > 90:
                click.secho("   • Cache nearly full - run 'eless cache --evict'", fg='yellow')
            if summary['cpu_percent'] > 90:
                click.secho("   • High CPU usage - consider pausing processing", fg='yellow')
            if summary['memory_percent'] < 50 and summary['cpu_percent'] < 50:
                click.secho("   • System resources available - can increase batch sizes", fg='green')
