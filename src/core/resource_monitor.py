...
class ResourceMonitor:
    ...
    def __init__(self, config: Dict):
        ...
        self.memory_warning_threshold = config.get('resource_limits', {}).get('memory_warning_percent', 80)
        self.memory_critical_threshold = config.get('resource_limits', {}).get('memory_critical_percent', 90)
        self.cpu_high_threshold = config.get('resource_limits', {}).get('cpu_high_percent', 85)
        
        # Minimum system requirements
        self.min_available_memory_mb = config.get('resource_limits', {}).get('min_memory_mb', 256)
        self.max_batch_size = config['embedding']['batch_size']
        self.min_batch_size = max(1, self.max_batch_size // 8)  # Never go below 1/8 of max
        
        # Monitoring history for trends
        self.history_size = 10
        self.metrics_history = []
        
        logger.info(f"ResourceMonitor initialized - Memory thresholds: {self.memory_warning_threshold}%/{self.memory_critical_threshold}%, "
    ...
    def should_throttle_processing(self) -> Tuple[bool, str]:
        ...
        if metrics.cpu_percent > self.cpu_high_threshold and metrics.memory_percent > self.memory_warning_threshold:
            return True, f"High CPU ({metrics.cpu_percent:.1f}%) + Memory ({metrics.memory_percent:.1f}%)"
    ...
    def get_adaptive_batch_size(self, current_batch_size: int) -> Tuple[int, str]:
        ...
        elif (metrics.memory_percent < 50 and 
              metrics.available_memory_mb > self.min_available_memory_mb * 2 and
    ...
    def get_system_summary(self) -> Dict:
        ...
        return {
            ...
            'recommended_batch_size': self.get_adaptive_batch_size(self.max_batch_size)[0],
    ...
    def log_system_status(self):
        ...
        logger.info(f"System Status - Memory: {summary['memory_percent']:.1f}% ({summary['memory_pressure']}), "
