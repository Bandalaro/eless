...
class Archiver:
    ...
    def save_chunks(self, file_hash: str, chunks_with_metadata: List[Dict]):
        ...
        try:
            ...
            logger.debug(f"Saved {len(chunks_with_metadata)} chunks for {file_hash[:8]} to {path.name}")
    ...
    def save_vectors(self, file_hash: str, vectors: np.ndarray):
        ...
        try:
            ...
            logger.debug(f"Saved vectors of shape {vectors.shape} for {file_hash[:8]} to {path.name}")
        except Exception as e:
