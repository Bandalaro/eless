...
class SmartCacheManager:
    ...
    def __init__(self, config: Dict):
        ...
        logger.info(f"SmartCacheManager initialized: max_size={self.max_size_mb}MB, max_files={self.max_files}")
    ...
    def should_evict(self) -> bool:
        ...
        if size_exceeded or files_exceeded:
            logger.info(f"Cache limits exceeded - Size: {current_size:.1f}MB/{self.max_size_mb}MB, "
    ...
    def evict_lru_items(self, target_size_mb: Optional[float] = None) -> List[str]:
        ...
        logger.info(f"Starting LRU eviction: {current_size:.1f}MB -> {target_size_mb:.1f}MB")
        ...
        logger.info(f"LRU eviction complete: evicted {len(evicted)} items, new size: {current_size:.1f}MB")
        return evicted
    ...
    def _remove_cached_files(self, file_hash: str) -> bool:
        ...
        removed = False
        patterns = [
            f"{file_hash}.chunks.pkl",
            f"{file_hash}.vectors.npy"
        ]
    ...
    def auto_maintain(self):
        ...
        if self.should_evict():
            ...
            logger.info(f"Auto-maintenance complete: cleaned {len(corrupted)} corrupted, "
