...
class ConfigWizard:
    ...
    def run_wizard(self) -> Dict[str, Any]:
        ...
        click.echo("This wizard will help you create an optimal configuration for your system.\n")
    ...
    def _show_system_info(self):
        ...
        click.echo(f"   Memory: {info['total_memory_gb']} GB total, {info['available_memory_gb']} GB available")
        click.echo(f"   Storage: {info['free_disk_gb']} GB free of {info['total_disk_gb']} GB total")
    ...
    def _get_storage_setup(self) -> Dict[str, str]:
        ...
        if use_single_dir:
            ...
            log_dir = click.prompt(
                "Log directory",
                default=str(home_dir / "eless_logs"),
                type=click.Path()
            )
            return {
                'type': 'separate',
    ...
    def _get_performance_profile(self, use_case: str) -> Dict[str, Any]:
        ...
        click.echo(f"\n⚡ Performance Profile:")
        click.echo(f"   Recommended: {profile} (based on {memory_gb}GB RAM, {cpu_cores} cores)")
    ...
    def _get_custom_performance(self) -> Dict[str, Any]:
        ...
        return {
            'embedding_batch_size': click.prompt(
                ...
                default=32, type=click.IntRange(1, 256)
            ),
            'database_batch_size': click.prompt(
                "Database batch size",
                default=64, type=click.IntRange(1, 512)
            ),
            'chunk_size': click.prompt(
                "Text chunk size (characters)",
                default=500, type=click.IntRange(100, 2000)
            ),
            'chunk_overlap': click.prompt(
                "Chunk overlap (characters)",
                default=50, type=click.IntRange(0, 500)
            ),
            'cache_max_size_mb': click.prompt(
                "Maximum cache size (MB)",
                default=1024, type=click.IntRange(100, 10000)
            )
        }
    ...
    def _get_database_selection(self) -> List[str]:
        ...
        if not selected:
            click.echo("At least one database is required. Selecting ChromaDB as default.")
            selected = ["chroma"]
        
        return selected
    
    def _build_configuration(self, use_case: str, storage: Dict, performance: Dict, databases: List[str]) -> Dict[str, Any]:
    ...
    def _build_database_connections(self, storage: Dict, databases: List[str]) -> Dict[str, Any]:
    ...
    def _show_final_config(self):
        ...
        click.echo(f"   Memory warning: {self.config['resource_limits']['memory_warning_percent']}%")
