...
class ElessPipeline:
    ...
    def run_process(self, source_path: str):
        ...
        try:
            ...
            embedded_chunk_generator = self.embedder.embed_and_archive_chunks(chunk_generator)
            ...
            logger.critical(f"A critical component failed to load or run (e.g., ModelWrapper): {e}")
    ...
    def run_resume(self):
        ...
        """
        logger.info("Resume command invoked. Resumption logic is handled internally by the Dispatcher.")
        logger.info("To resume, typically the original source path would be re-provided or read from a session log.")
