...
class TestConfigValidation(unittest.TestCase):
    ...
    def test_invalid_database_config(self):
        ...
        self.config['database']['type'] = 'unsupported_db'
        with self.assertRaises(ValueError):
            self.config_loader.validate_config(self.config)

        # Test invalid batch size
        self.config['database']['batch_size'] = -1
        with self.assertRaises(ValueError):
            self.config_loader.validate_config(self.config)
...
class TestConfigValidation(unittest.TestCase):
    ...
    def test_minimal_config(self):
        ...
        minimal_config = {
            ...
            "cache": {"directory": "/tmp/eless_cache"},
            "database": {"type": "sqlite", "path": "/tmp/eless.db"},
        }
        config_path = self.save_config(minimal_config)
        loaded_config = self.config_loader.load_config(config_path)
        self.assertIsNotNone(loaded_config)
    ...
    def test_config_environment_variables(self):
        ...
        # Load config with environment variable support
        config = self.config_loader.load_config_with_env(self.save_config(self.config))
    ...
    def test_config_merge(self):
        """Test configuration merging."""
        # Create base config
        base_config = {"embedding": {"batch_size": 32}, "chunking": {"chunk_size": 512}}
    ...
    def test_config_defaults(self):
        """Test default configuration values."""
        # Load minimal config
        minimal_config = {"embedding": {"model": "test-model"}}
